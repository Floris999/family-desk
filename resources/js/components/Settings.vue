...
const updateProfile = async (event) => {
  ...
  formData.append("name", event.target.name.value || userName);
  formData.append("email", event.target.email.value || userEmail);
  formData.append("mood", event.target.about.value);
  formData.append("cijfer", event.target.cijfer.value || userCijfer);
  formData.append("role", event.target.role.value || userRole);
  if (event.target["file-upload"].files[0]) {
    formData.append("photo", event.target["file-upload"].files[0]);
  }

  try {
    const response = await axios.post("/settings/update", formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });

    // Update de authStore met de nieuwe gegevens
        ...
        <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
          <div class="col-span-full">
            ...
            <div class="mt-2">
              <textarea
                ...
                id="about"
                rows="3"
                :placeholder="userMood"
                class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
              ></textarea>
            </div>
            <p class="mt-3 text-sm/6 text-gray-600">Don't worry be happy.</p>
          </div>
          ...
          <div class="col-span-full">
            <div class="mt-2">
              ...
              <img
                class="size-35 rounded-full mt-4 mb-4"
                :src="userPicture"
                alt=""
              />
            </div>
